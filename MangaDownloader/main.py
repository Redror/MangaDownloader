...
from sites import sussytoons, mangalivre, sakuramangas, comick, manhastro
...
def main():
    ...
    while True:
        ...
        is_problematic_site = "comick.io" in obra_url or "manhastro.net" in obra_url
        ...
        # Inicia um novo driver para o SussyToons se necessário (caso o primeiro site não tenha usado Selenium)
        needs_sussy_driver = any(
            (site_handler == "sussy_hybrid" and cap.get('cap_disponivel', True)) for cap in caps_para_baixar
        )
        if needs_sussy_driver and driver_selenium is None:
             driver_selenium = setup_selenium_driver(run_headless=True) # Sussy sempre pode ser headless
        ...
        for i, cap_info in enumerate(caps_para_baixar):
            ...
            chapter_number = cap_info['cap_numero'] # Este valor é um float (ex: 1.0, 2.5)

            # Formata o número para exibição na mensagem do console
            if chapter_number.is_integer():
                # Se for um número inteiro (1.0, 2.0), mostra como 1, 2
                display_number = int(chapter_number)
            else:
                # Se for fracionado (2.5), mantém o decimal
        ...
        # Fecha o navegador ao final de CADA obra.
        if driver_selenium:
